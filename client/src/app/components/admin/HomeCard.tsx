...
function HomeCard({ title, description, count, bgColor }: HomeCardProps) {
  ...
  return (
    ...
    >
      <div className={`absolute bottom-0 ${isMouseEnter ? "h-full w-full" : "h-0 w-0"} duration-300 rounded-full bg-opacity-5 bg-gradient-to-t from-black to-neutral-300`}></div>
      <p>{title}</p>
